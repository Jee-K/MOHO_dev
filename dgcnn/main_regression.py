...
def freeze_layers_transfer_learning(model):
    for param in model.parameters():
        param.requires_grad = False
    for name, param in model.named_parameters():
        if any(x in name for x in ['conv8', 'conv9', 'conv10', 'conv11']):
            param.requires_grad = True

def load_state_dict_with_prefix_fix(model, model_path):
...
def train(args, io):
    ...
    nn.init.kaiming_normal_(model.conv11.weight)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='dgcnn_regression', metavar='N',
                        choices=['dgcnn', 'dgcnn_regression'],
