...
def load_data_partseg_from_txt(partition, num_points=30000):
    """
    Args:
        partition (str): 'train', 'test', 'trainval', etc.
        num_points (int, optional): If set, randomly sample to this number.
        
    Returns:
        all_data: np.ndarray [N_samples, N_points, 6] -> (x,y,z,r,g,b)
        all_label: np.ndarray [N_samples, 1] -> class id
        all_seg: np.ndarray [N_samples, N_points] -> per-point part ids
    """
    
    BASE_DIR = os.path.dirname("/data")  # same as before
    DATA_DIR = os.path.join('/data/','DGCNN_datatest')  # folder where your .txts are stored
    print(DATA_DIR)
    # Match partition — for example, "data_txt/train/*.txt"
    file_list = glob.glob(os.path.join(DATA_DIR, partition, '*.txt'))
    file_list.sort()
    ...
    for txt_path in file_list:
        # Load file: shape [N_points, 4] (x y z label)
        pc_data = np.loadtxt(txt_path).astype(np.float32)
        ...
        if pc_data.shape[1] == 4:
            ...
            coords_rgb = pc_data[:, 0:3]
            seg_labels = np.zeros(coords_rgb.shape[0], dtype=np.int64)  # dummy labels if not provided

        # ---------- OPTIONAL: sample to fixed num_points ----------
        if num_points is not None:
            if coords_rgb.shape[0] >= num_points:
                choice = np.random.choice(coords_rgb.shape[0], num_points, replace=False)
            else:
                choice = np.random.choice(coords_rgb.shape[0], num_points, replace=True)
            coords_rgb = coords_rgb[choice, :]
            seg_labels = seg_labels[choice]

        # Here `label` is the object class index  
        # if you have it encoded inside the file name or folder, extract it:
        # Example: train/mug_001.txt -> class name 'mug' -> class_id
        class_name = os.path.basename(txt_path).split('_')[0]
        # Map to int via your cat2id mapping
        class_id = cat2id_map[class_name] if 'cat2id_map' in globals() else 0  
        label_arr = np.array([class_id], dtype=np.int64)  # shape (1,)

        all_data.append(coords_rgb)
        all_label.append(label_arr)
        all_seg.append(seg_labels)

    all_data = np.stack(all_data, axis=0)    # [N_samples, N_points, 6]
    all_label = np.stack(all_label, axis=0)  # [N_samples, 1]
    all_seg = np.stack(all_seg, axis=0)      # [N_samples, N_points]

    return all_data, all_label, all_seg

class MOHOReg(Dataset):
    def __init__(self, num_points=None, partition='train', class_choice=None):
        self.data, self.label, self.seg = load_data_partseg_from_txt(partition) #load_data_partseg(partition)
    ...
    def __getitem__(self, item):
        # Take first num_points points
        pointcloud = self.data[item][:self.num_points]
        label = self.label[item]   # still returned for API compatibility
        seg = self.seg[item][:self.num_points]

        # Make sure regression targets are floats
        seg = seg.astype(np.float32)

        # Shuffle points and targets during training
